...
import { visitAlcada } from './alcada-visit.js';
...
async function main(): Promise<void> {
  ...
  let cards = CARD_FIXTURES.filter((c) => c.phase === 5);
  ...
  for (const card of cards) {
    ...
    try {
      console.log('   Realizando aprovação de alçada...');
      const result = await visitAlcada(context, card, clientId, phaseSlug, BASE_URL, LOCALE, FLOW_ID);
      const status = result.concludedOk ? '✓ concluído' : '⚠️ conclusão incerta';
      console.log(`   ${status}`);
      ok++;
    } catch (err) {
