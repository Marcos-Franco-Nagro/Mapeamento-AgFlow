...
// Card específico para fluxo de caixa
const CARD_ID_OVERRIDE = process.env.CARD_ID ?? '6a05bfccf3e4ddc495eb845e';
...
async function saveEndpoints(requests: CapturedRequest[], folder: string): Promise<void> {
  const endpointsDir = path.join('vault', 'endpoints', 'modulo-cash-flow', folder);
