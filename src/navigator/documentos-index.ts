...
import { visitDocumentos } from './documentos-visit.js';
...
async function main(): Promise<void> {
  ...
  for (const card of cards) {
    ...
    try {
      console.log('   Navegando no módulo de Documentos...');
      const result = await visitDocumentos(context, card, BASE_URL, LOCALE, FLOW_ID);
      await saveEndpoints(result.requestsNavPastas,   'navegacao-pastas');
      await saveEndpoints(result.requestsForm1Upload, 'forma1-add-doc-diretonapasta');
      await saveEndpoints(result.requestsForm2Anexar, 'forma2-novo-documento-anexar');
      console.log(
        `   ✓ ok — nav: ${result.requestsNavPastas.length} ep | ` +
        `forma1: ${result.requestsForm1Upload.length} ep | ` +
        `forma2: ${result.requestsForm2Anexar.length} ep`,
      );
      ok++;
    } catch (err) {
