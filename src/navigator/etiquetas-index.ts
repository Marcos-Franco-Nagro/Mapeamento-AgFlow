...
import { visitEtiquetas } from './etiquetas-visit.js';
...
async function saveEndpoints(requests: CapturedRequest[], folder: string): Promise<void> {
  const dir = path.join('vault', 'endpoints', 'etiquetas', folder);
  ...
  for (const req of requests) {
    try {
      ...
      await fs.writeFile(
        file,
        `---\nmethod: ${req.method}\nurl: "${req.url}"\nstatus: ${req.status ?? 'unknown'}\ntags: [endpoint, agflow, card, etiquetas, ${folder}]\n---\n\n# ${req.method} ${req.url}\n\n## Observações\n\n-\n`,
        'utf-8',
      );
    } catch { /* ignorar */ }
  }
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const result = await visitEtiquetas(context, CARD_URL);

    await saveEndpoints(result.requestsNavegacao, 'navegacao');
    await saveEndpoints(result.requestsAdicionarEtiqueta, 'adicionar-etiqueta');
    await saveEndpoints(result.requestsRemoverEtiqueta, 'remover-etiqueta');

    console.log('\n✅ Concluído.');
    console.log(`   Screenshots: vault/screenshots/etiquetas/`);
    console.log(`   Endpoints:   vault/endpoints/etiquetas/`);
    console.log(`     navegacao:          ${result.requestsNavegacao.length} request(s)`);
    console.log(`     adicionar-etiqueta: ${result.requestsAdicionarEtiqueta.length} request(s)`);
    console.log(`     remover-etiqueta:   ${result.requestsRemoverEtiqueta.length} request(s)`);
  } catch (err) {
