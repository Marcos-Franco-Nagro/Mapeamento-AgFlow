...
import { visitFiltroDuplo } from './filtro-duplo-visit.js';
...
async function saveEndpoints(requests: CapturedRequest[], folder: string): Promise<void> {
  const dir = path.join('vault', 'endpoints', 'filtros', 'filtro-duplo', folder);
  ...
  for (const req of requests) {
    try {
      ...
      await fs.writeFile(
        file,
        `---\nmethod: ${req.method}\nurl: "${req.url}"\nstatus: ${req.status ?? 'unknown'}\ntags: [endpoint, agflow, filtros, filtro-duplo, ${folder}]\n---\n\n# ${req.method} ${req.url}\n\n## Observações\n\n-\n`,
        'utf-8',
      );
    } catch { /* ignorar */ }
  }
...
async function main(): Promise<void> {
  ...
  try {
    console.log('🔍 Filtro duplo — Limite aprovado pelo analista = R$ 150.000,00 E Título do card Contém "teste"');

    const result = await visitFiltroDuplo(context, BASE_URL, LOCALE, FLOW_ID);
    ...
    console.log('\n✅ Concluído.');
    console.log(`   Screenshots: vault/screenshots/filtros/filtro-duplo/`);
    console.log(`   Endpoints:   vault/endpoints/filtros/filtro-duplo/`);
