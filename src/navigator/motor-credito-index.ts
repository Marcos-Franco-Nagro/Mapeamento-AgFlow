...
import { visitMotorCredito } from './motor-credito-visit.js';
...
async function main(): Promise<void> {
  ...
  for (const card of cards) {
    ...
    try {
      console.log('   Analisando motor de crédito...');
      const result = await visitMotorCredito(context, card, phaseSlug, BASE_URL, LOCALE, FLOW_ID);
      await saveEndpoints(result.requestsMotor, phaseSlug, 'motor-credito');
      console.log(`   ✓ analisado — ${result.requestsMotor.length} endpoint(s)`);
      ok++;
    } catch (err) {
