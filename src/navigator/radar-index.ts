...
import { visitRadar } from './radar-visit.js';
...
async function main(): Promise<void> {
  ...
  for (const card of cards) {
    ...
    try {
      console.log('   Processando painel de alertas (Radar)...');
      const result = await visitRadar(context, card, phaseSlug, BASE_URL, LOCALE, FLOW_ID);

      // Salvar endpoints capturados no vault
      const endpointsDir = path.join('vault', 'endpoints', phaseSlug, 'painel-de-alertas');
      await fs.mkdir(endpointsDir, { recursive: true });
      for (const req of result.requestsRadar) {
        try {
      ...
      const status = result.processedOk ? '✓ processado' : '⚠️ processamento incerto';
      console.log(`   ${status} — ${result.requestsRadar.length} endpoint(s) capturado(s)`);
      ok++;
    } catch (err) {
