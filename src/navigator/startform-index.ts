...
import { visitStartform } from './startform-visit.js';
...
async function saveEndpoints(requests: CapturedRequest[]): Promise<void> {
  const endpointsDir = path.join('vault', 'endpoints', 'startform');
  ...
  for (const req of requests) {
    try {
      ...
      await fs.writeFile(
        file,
        `---\nmethod: ${req.method}\nurl: "${req.url}"\nstatus: ${req.status ?? 'unknown'}\ntags: [endpoint, agflow, startform]\n---\n\n# ${req.method} ${req.url}\n\n## Observações\n\n-\n`,
        'utf-8',
      );
    } catch { /* ignorar */ }
  }
...
async function main(): Promise<void> {
  ...
  try {
    console.log('\n📋 Preenchendo StartForm...');
    const result = await visitStartform(context, BASE_URL, LOCALE, FLOW_ID);
    await saveEndpoints(result.requestsStartform);
    console.log(`✓ Formulário submetido — ${result.requestsStartform.length} endpoint(s) capturado(s)`);
    console.log('\n✅ Concluído.');
  } catch (err) {
